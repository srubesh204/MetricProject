        ...
        const itemsData = dcPartyItems.map((item, index) => {
          let tableRow = `
              <tr>
                  ...
                  Make: ${item.itemMake ? item.itemMake : "-"} Sr.No: ${item.itemMFRNo ? item.itemMFRNo : "-"} Cal. Frequency: ${(item.itemCalFreInMonths && item.itemCalFreInMonths !== "undefined" ? item.itemCalFreInMonths : "-") + " " + (item.itemCalFrequencyType && item.itemCalFrequencyType !== "undefined" ? item.itemCalFrequencyType : "")}</td>
